...
export function MainPage({ theme, onSelectFortune, onSelectDharma, onThemeChange }: MainPageProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-2 sm:p-4"
         style={{ 
           background: `linear-gradient(to bottom right, ${colors.secondary}90, black)`
         }}>
      <div className="w-full max-w-4xl mx-auto p-4 sm:p-6 md:p-8 rounded-2xl backdrop-blur-md bg-black/40 border-2 sm:border-4 shadow-2xl"
        ...
        >
          <motion.h1 
            className="text-3xl sm:text-4xl md:text-5xl font-serif mb-4 sm:mb-6 md:mb-8 font-bold drop-shadow-lg tracking-wide"
            ...
            >
              ...
              <div className="relative z-10 flex flex-col items-center">
                {isLoading && activeButton === 'fortune' ? (
                  <Loader2 className="w-8 h-8 sm:w-12 sm:h-12 animate-spin" style={{ color: colors.primary }} />
                ) : (
                  <div className="w-16 h-16 sm:w-20 sm:h-20 mb-4 sm:mb-6 flex items-center justify-center rounded-full bg-gradient-to-br from-purple-600/20 to-indigo-600/20">
                    <Scroll className="w-8 h-8 sm:w-12 sm:h-12" style={{ color: colors.primary }} />
                  </div>
                )}
              ...
              <div className="relative z-10 flex flex-col items-center">
                {isLoading && activeButton === 'dharma' ? (
                  ...
                  <div className="w-16 h-16 sm:w-20 sm:h-20 mb-4 sm:mb-6 flex items-center justify-center rounded-full bg-gradient-to-br from-amber-600/20 to-yellow-600/20">
                    <BookOpen className="w-8 h-8 sm:w-12 sm:h-12" style={{ color: colors.primary }} />
                  </div>
                )}
          ...
          <div className="mt-8 flex justify-center">
            <ThemeSelector currentTheme={theme?.id || themes[0].id} onSelectTheme={onThemeChange || (() => {})} />
          </div>
              ...
              >
                <div className="flex items-center space-x-2">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 flex items-center justify-center rounded-full bg-gradient-to-br from-green-600/20 to-emerald-600/20">
                    <BookOpen className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" style={{ color: colors.primary }} />
                  </div>
                  <h3 className="text-sm sm:text-base md:text-lg font-serif font-bold" style={{ color: colors.primary }}>
                    Guide
                  </h3>
                </div>
                {showEducation ? (
                  <ChevronUp className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: colors.primary }} />
                ) : (
                  <ChevronDown className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: colors.primary }} />
                )}
              </motion.button>
            </div>
                  ...
                  <div className="p-4 sm:p-5 md:p-6 rounded-xl bg-black/40 backdrop-blur-sm border-2"
                    ...
                    <div className="space-y-4 sm:space-y-6">
                      ...
                      <div className="space-y-2">
                        <h4 className="text-base sm:text-lg font-serif font-bold" style={{ color: colors.primary }}>
